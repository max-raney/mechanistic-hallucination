...
# Step1: Load Meta-Llama-3-8B model
def load_llama_model():
    ...
    tokenizer = AutoTokenizer.from_pretrained(llama_path, token=tk)

    print("Loaded Llama model and tokenizer.")
    return model, tokenizer


# Step2: Wrap Llama model to capture its hidden states on all layers
def wrap_llama_with_hooks(model, layer_idx_to_hook):
    hidden_states = []
...
def scan_all_layers(model, tokenizer, prompt, direction_vector=None):
    ...
    for i in range(num_layers):
        activations, hook = wrap_llama_with_hooks(model, i)
...
scores = scan_all_layers(llama_model, llama_tokenizer, prompt, direction_vector=dangerous_intent_dir)
...
plt.title("Prompt Activation Across Transformer Layers of the Meta-Llama-3-8B model")
plt.grid(True)
plt.show()
